...
import "./sideNav.scss";
import {sideNavBottomItems, sideNavMiddleItems, sideNavTopItems} from "../../types/dataModels";
    ...
    return (
        ...
        <div className="side-nav-bottom">
          {sideNavBottomItems.map(({ Icon, name, path },key) => {
            return (
              <div key={key} className="side-nav-bottom-options" onClick={()=>{history.push(path)}} >
                <Icon />
                <h3>{name}</h3>
              </div>
            );
